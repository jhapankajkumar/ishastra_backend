...
export type DivergenceCategory = 'REGULAR' | 'HIDDEN' | 'VOLUME';
export type DivergenceStrength = 'WEAK' | 'MODERATE' | 'STRONG';
export type DivergenceSignal = 'BULLISH' | 'BEARISH' | 'NEUTRAL';
...
export interface DivergenceSummary {
    ...
    hiddenDivergences: number;
    regularDivergences: number;
}
 ...
 */
export function detectMomentumDivergences(
    ohlcData: OHLCData[], 
    ...
    if (!ohlcData || ohlcData.length < 20) {
        return { 
            ...
            summary: {
                totalDivergences: 0,
                bullishDivergences: 0,
                bearishDivergences: 0,
                hiddenDivergences: 0,
                regularDivergences: 0
            }
        };
    }
...
export function detectRSIDivergence(priceData: OHLCData[], rsiData: number[]): Divergence {
...
export function detectMACDDivergence(priceData: OHLCData[], macdData: TechnicalIndicators['macd']): Divergence {
...
export function detectStochasticDivergence(priceData: OHLCData[], stochData: TechnicalIndicators['stochastic']): Divergence {
...
export function detectVolumeDivergence(priceData: OHLCData[]): Divergence {
...
export function findLocalHighs(data: number[], minPeriods: number = 2): LocalPoint[] {
    const highs: LocalPoint[] = [];
...
export function findLocalLows(data: number[], minPeriods: number = 2): LocalPoint[] {
    const lows: LocalPoint[] = [];
    for (let i = minPeriods; i < data.length - minPeriods; i++) {
        let isLow = true;
        for (let j = i - minPeriods; j <= i + minPeriods; j++) {
            if (j !== i && data[j] <= data[i]) {
...
export function calculateDivergenceStrength(pricePoints: LocalPoint[], indicatorPoints: LocalPoint[]): DivergenceStrength {
    ...
    const strength: DivergenceStrength = pricePercent > 0.05 && indicatorDiff > 5 ? 'STRONG' : 
...
// Calculate overall divergence signal
export function calculateDivergenceSignal(divergences: Divergence[]): DivergenceResult {
...
function calculateRSI(ohlcData: OHLCData[], period: number = 14): number[] {
...
function calculateMACD(ohlcData: OHLCData[]): TechnicalIndicators['macd'] {
...
function calculateStochastic(ohlcData: OHLCData[], kPeriod: number = 14, dPeriod: number = 3): TechnicalIndicators['stochastic'] {
    const k: number[] = [];
