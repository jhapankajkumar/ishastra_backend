...
export interface VolumeMetrics {
  ...
  volumeStrength: 'VERY_HIGH' | 'HIGH' | 'NORMAL' | 'LOW' | 'VERY_LOW';
  avgVolume20: number;
...
export interface MicrostructureAnalysis {
  ...
  spreadProxy: number;
  openingDrive: 'BULLISH' | 'BEARISH' | 'NEUTRAL';
 ...
 */
export function calculateVWAP(ohlcData: OHLCData[]): number | null {
 ...
 * ✅ ACHIEVABLE: Uses public OHLCV data
 */
export function calculateVolumeProfile(ohlcData: OHLCData[], bins: number = 20): VolumeProfileResult {
  if (!ohlcData || ohlcData.length === 0) {
  ...
  const volumeProfile: VolumeProfileLevel[] = Array(bins).fill(0).map((_, i) => ({
 ...
 */
export function analyzeVolumeMetrics(ohlcData: OHLCData[]): VolumeMetrics {
  ...
  let volumeStrength: 'VERY_HIGH' | 'HIGH' | 'NORMAL' | 'LOW' | 'VERY_LOW' = 'NORMAL';
 ...
 */
export function analyzeMicrostructureLite(ohlcData: OHLCData[]): MicrostructureAnalysis {
  ...
  let openingDrive: 'BULLISH' | 'BEARISH' | 'NEUTRAL' = 'NEUTRAL';
