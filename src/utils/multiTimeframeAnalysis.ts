...
export interface OverallConfluence {
    score: number;
    interpretation: string;
    agreement: string;
    timeframeBreakdown: TimeframeAgreement[];
    confidence: number;
}
...
export interface MultiTimeframeResult {
    symbol: string;
    timestamp: string;
    timeframes: { [timeframe: string]: TimeframeAnalysis };
    confluenceScores: { [timeframe: string]: ConfluenceScore };
...
export class MultiTimeframeAnalysis {
   ...
   * @param timeframes - Array of timeframes ['1d', '1wk', '1mo']
   * @returns Multi-timeframe analysis results
   */
  static async analyzeMultipleTimeframes(
    symbol: string, 
    timeframes: string[] = ['1d', '1wk']
  ): Promise<MultiTimeframeResult> {
    const results: { [timeframe: string]: TimeframeAnalysis } = {};
    const confluenceScores: { [timeframe: string]: ConfluenceScore } = {};
   ...
   * @param timeframe - Timeframe (1d, 1wk, 1mo)
   * @returns Start and end dates
   */
  static getPeriodForTimeframe(timeframe: string): TimeframePeriod {
    const now = new Date();
    const periods: { [key: string]: TimeframePeriod } = {
      '1d': {
   ...
   * @param analysis - Analysis results for timeframe
   * @returns Confluence score breakdown
   */
  static calculateConfluenceScore(analysis: TimeframeAnalysis): ConfluenceScore {
    let score = 0;
    ...
    if (analysis.signals?.systems) {
      ...
      const bullishSystems = systemSignals.filter((s: any) => s.signal === 'BUY' || s.signal === 'STRONG_BUY').length;
      const bearishSystems = systemSignals.filter((s: any) => s.signal === 'SELL' || s.signal === 'STRONG_SELL').length;
    ...
    if (analysis.advancedPatterns && analysis.advancedPatterns.length > 0) {
      analysis.advancedPatterns.forEach((pattern: any) => {
   ...
   * @param confluenceScores - Scores for each timeframe
   * @returns Overall confluence analysis
   */
  static calculateOverallConfluence(confluenceScores: { [timeframe: string]: ConfluenceScore }): OverallConfluence {
    ...
    let weightedScore = 0;
    let totalWeight = 0;
    const agreements: TimeframeAgreement[] = [];
   ...
   * @param score - Confluence score (0-100)
   * @returns Interpretation
   */
  static interpretConfluenceScore(score: number): string {
   ...
   * @param agreement - Timeframe agreement
   * @returns Confidence percentage
   */
  static calculateConfidenceLevel(score: number, agreement: string): number {
    let baseConfidence = Math.abs(score - 50) * 2; // Distance from neutral
    
    // Boost confidence based on timeframe agreement
    const agreementBoost: { [key: string]: number } = {
  ...
  /**
   * Generate multi-timeframe recommendation
   * @param results - Analysis results for all timeframes
   * @param confluence - Overall confluence analysis
   * @returns Trading recommendation
   */
  static generateMultiTimeframeRecommendation(
    results: { [timeframe: string]: TimeframeAnalysis }, 
    confluence: OverallConfluence
   ...
   * @returns Risk level
   */
  static calculateRiskLevel(action: string, confidence: number, agreement: string): string {
...
module.exports.default = MultiTimeframeAnalysis;
