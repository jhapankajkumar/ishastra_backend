...
export type SentimentConfidence = 'NONE' | 'LOW' | 'MEDIUM' | 'HIGH';
export type SentimentTrend = 'BULLISH' | 'BEARISH' | 'NEUTRAL';
export type SignificanceLevel = 'LOW' | 'MEDIUM' | 'HIGH';
export type SupportLevel = 'WEAK' | 'NEUTRAL' | 'STRONG';
...
export interface SentimentImpact {
    shouldAdjustConfidence: boolean;
...
export interface NewsAnalyzer {
    analyzeNews: (symbol: string) => Promise<NewsSentiment>;
}

export interface SocialAnalyzer {
    analyzeSocial: (symbol: string) => Promise<SocialSentiment>;
}

export interface InsiderAnalyzer {
    analyzeInsiderTrades: (symbol: string) => Promise<InsiderAnalysis>;
}
...
export interface SentimentAnalysisResult {
    ...
    reliability: string;
    analysisType: string;
    fusion: SentimentFusionResult;
    dataQuality: string;
    disclaimer: string;
}

export class RealisticSentimentEngine {
    private newsSentimentAnalyzer: NewsAnalyzer | null;
    private socialSentimentAnalyzer: SocialAnalyzer | null;
    private insiderTradeAnalyzer: InsiderAnalyzer | null;
    ...
    async initializeRealisticAnalyzers(): Promise<void> {
        ...
        try {
            ...
            console.error('❌ Failed to initialize Sentiment Engine:', error);
        }
    }

    private createNewsAnalyzer(): NewsAnalyzer {
        // Basic news sentiment using simple keyword analysis
        return {
            analyzeNews: async (symbol: string): Promise<NewsSentiment> => {
    ...
    private createSocialAnalyzer(): SocialAnalyzer {
        // Basic social media sentiment (Reddit, basic sources)
        return {
            analyzeSocial: async (symbol: string): Promise<SocialSentiment> => {
    ...
    private createInsiderAnalyzer(): InsiderAnalyzer {
        // ✅ SEC filing analysis - public data available
        return {
            analyzeInsiderTrades: async (symbol: string): Promise<InsiderAnalysis> => {
                ...
                try {
                    ...
                    console.error('❌ Insider trade analysis failed:', error);
                    return analysis;
                }
            }
        };
     ...
     */
    async analyzeSentimentFusion(symbol: string): Promise<SentimentFusionResult> {
        //console.log(`📰 Analyzing realistic sentiment for ${symbol}...`);
        
        const fusionResult: SentimentFusionResult = {
        ...
        try {
            if (!this.newsSentimentAnalyzer || !this.socialSentimentAnalyzer || !this.insiderTradeAnalyzer) {
            ...
            // Extract results
            const newsData = newsResult.status === 'fulfilled' ? newsResult.value : { score: 0.5, confidence: 'NONE' as SentimentConfidence, newsCount: 0, positiveKeywords: 0, negativeKeywords: 0, sources: [] };
            const socialData = socialResult.status === 'fulfilled' ? socialResult.value : { score: 0.5, confidence: 'NONE' as SentimentConfidence, sources: [], bullishMentions: 0, bearishMentions: 0, totalMentions: 0, trend: 'NEUTRAL' as SentimentTrend };
            const insiderData = insiderResult.status === 'fulfilled' ? insiderResult.value : { score: 0.5, confidence: 'NONE' as SentimentConfidence, recentTrades: 0, buyTrades: 0, sellTrades: 0, netSentiment: 'NEUTRAL' as SentimentTrend, lastTradeDate: null, significance: 'LOW' as SignificanceLevel };
            ...
            console.error('❌ Sentiment fusion failed:', error);
     ...
     */
    assessSentimentImpact(fusionResult: SentimentFusionResult, currentDecision: any): SentimentImpact {
        const impact: SentimentImpact = {
        ...
        try {
            ...
            console.error('❌ Sentiment impact assessment failed:', error);
            return impact;
        }
    }

    private getConfidenceMultiplier(confidence: SentimentConfidence): number {
        switch (confidence) {
     ...
     */
    async analyze(symbol: string, sector: string = 'UNKNOWN'): Promise<SentimentAnalysisResult> {
